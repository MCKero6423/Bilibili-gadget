...
def download_single_audio(url: str, cookies: Dict):
    ...
    try:
        cmd = [
            'yt-dlp',
            '--cookies', 'cookies.txt',  # 直接使用当前目录的cookies.txt
            '-f', 'ba',
            '--no-playlist',
...
def main():
    ...
    # 检查cookies.txt是否存在
    if not os.path.exists('cookies.txt'):  # 直接检查当前目录
    ...
    while True:
        ...
        if choice == '1':
            ...
            # 获取视频热评
            input_text = input("请输��视频URL或BV号: ").strip()
            ...
            while True:
                ...
                if live_choice == '1':
                    ...
                    mid = input("请输入用户UID: ").strip()
                    if mid.isdigit():
                        live_room.get_user_live_status(mid)
                    else:
                        print("请输入有效的用户UID！")
                
                elif live_choice == '3':
                    uid = input("请输入主播UID: ").strip()
                    if uid.isdigit():
                    ...
                    print("请输入直播间号，多个房间号用空格分隔（如：1 3 5）：")
                    room_ids = input().strip().split()
                    if all(rid.isdigit() for rid in room_ids):
                        live_room.get_room_base_info(room_ids)
                    else:
                        print("请输入有效的直播间号！")
                
                elif live_choice == '5':
                    print("请输入要查询的主播UID，多个UID用空格分隔（如：672328094 2 3）：")
                    uids = input().strip().split()
                    if all(uid.isdigit() for uid in uids):
                        live_room.get_batch_live_status(uids)
                    else:
                        print("请输入有效的主播UID！")
                    ...
                    room_id = input("请输入直播间号: ").strip()
                    if room_id.isdigit():
                        live_room.get_history_danmaku(room_id)
                    else:
                        print("请输入有效的直播间号！")
                    ...
                    room_id = input("请输入直播间号: ").strip()
                    if room_id.isdigit():
                        live_room.get_play_info(room_id)
                    else:
                        print("请输入有效的直播间号！")
                    ...
                    room_id = input("请输入直播间号: ").strip()
                    if room_id.isdigit():
                        live_room.get_room_anchor_info(room_id)
                    else:
                        print("请输入有效的直播间号！")
    ...
    try:
        ...
        if data['code'] == 0:
            if 'data' in data and data['data']:
                ...
                # 官方认证信息
                if info['official_verify']['type'] != -1:
                    verify_type = "个��认证" if info['official_verify']['type'] == 0 else "机构认证"
                    print(f"认证类型: {verify_type}")
                    print(f"认证说明: {info['official_verify']['desc']}")
...
class LiveRoom:
    ...
    def get_batch_live_status(self, uids: list) -> None:
        """批量查询直播间状态"""
        # 原get_batch_live_status函数的内容
        
    def get_history_danmaku(self, room_id: str) -> None:
        """获取直播间历史弹幕"""
        # 原get_live_history_danmaku函数的内容
        
    def get_play_info(self, room_id: str) -> None:
        """获取直播间播放信息"""
        # 原get_room_play_info函数的内容
        
    def get_room_anchor_info(self, room_id: str) -> None:
        """获取直播间主播详细信息"""
        # 原get_room_anchor_info函数的内容
