        ...
        if bvid:
            ...
            if len(pages) > 1:
                ...
                if valid_pages:
                    ...
                    download_single_audio(url, cookies)
            else:
                # 单P视频直接下载
                download_single_audio(url, cookies)
    except Exception as e:
...
def main():
    ...
    while True:
        ...
        if choice == '1':
            ...
            if url.startswith(('https://www.bilibili.com', 'https://b23.tv')):
                try:
                    # 获取视频cid
                    bvid = extract_bvid(url)
                    if bvid:
...
def get_danmaku(cid: str, segment_index: int = 1) -> None:
    """获取视频弹幕"""
    try:
    ...
    try:
        ...
        if resp.status_code == 200:
            ...
            for elem in danmaku_seg.elems:
                ...
                # 转换颜色为十六进制
                color = f"#{elem.color:06x}"
                
                print(f"时间: {minutes:02d}:{seconds:02d}")
                print(f"内容: {elem.content}")
                print(f"类型: {mode_type}")
                print(f"颜色: {color}")
                print(f"字号: {elem.fontsize}")
                print(f"发送时间: {time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(elem.ctime))}")
                print("-" * 50)
